...
type PhotosContextType = {
  ...
  updatePhoto: (id: string, updates: { takenAt?: string }) => Promise<void>;
...
export function PhotosProvider({ children }: { children: React.ReactNode }) {
  ...
  const updatePhoto = useCallback(async (id: string, updates: { takenAt?: string }) => {
  ...
  const deletePhotos = useCallback(async (ids: string[]) => {
    ...
    const updated = photos.filter((p) => !idSet.has(p.id));
    setPhotos(updated);
    await AsyncStorage.setItem(PHOTOS_KEY, JSON.stringify(updated));
  }, [photos]);
